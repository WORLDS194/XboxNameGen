...
rnn.load_state_dict(torch.load(load_path, map_location=torch.device('cpu')))device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

rnn.load_state_dict(torch.load(load_path, map_location=device))
rnn.to(device)
